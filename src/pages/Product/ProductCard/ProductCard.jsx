...
function ProductCard(props) {
	...
	const getCart = () => {
		axios
			.get('http://127.0.0.1:8000/api/v1/carts')
			.then((res) => {
					...
					return (
						<div className={`card col-lg-3 ${styles.product}`} key={p.id}>
							<div className="card-body">
								...
								<hr />
								<p className={styles.product__cap}>
									Capacity : {p.capacity} {p.type}
								</p>
								<hr />
							...
							<div className={`input-group mb-3 ${styles.productCart}`}>
								<div className="input-group-prepend">
									<span className="input-group-text" onClick={onChangeVal(p.id, p.cart - 1)}>
										-
									</span>
								</div>
								<input
